...
> **前置**：Phase A～K 功能清单已完成（484+ 单测）；见 [PROJECT_STATUS.md](./00-PROJECT_STATUS.md)。  
> **目标**：**不扩新模块**，把 stub / 未验证能力 **做深、做真、串成故事**，支撑面试讲解与工程 credibility。  
> **ROI 优先级**：[phase-l-priority-roi.md](./02-phase-l-01-priority-roi.md)  
> **Issue 正文**：[issues-backlog-phase-l.md](./99-issues-backlog-phase-l.md)（GitHub #37～#47 对应 backlog #53～#63）。  
> **面试稿**：[interview-narrative.md](./90-interview-narrative.md)
...
| **文档滞后** | `00-roadmap.md` 已知限制、`90-gap-analysis-diagram.md` 与 Phase I～K 矛盾 | 口述与仓库现状不一致，易被穿帮 |
...
5. **面试叙事驱动**：每个 Wave 对应 SOP 文档中的一条大厂故事（见 [enterprise-ai-platform-sop.md](./02-phase-e-sop.md)）。

---

## 3.5 ROI 优先级（面试 + 工程深度路线）

完整对照表、状态矩阵见 **[phase-l-priority-roi.md](./02-phase-l-01-priority-roi.md)**。
...
**动机**：`00-roadmap.md` §已知限制仍写「无 MCP / 无 HITL」；`90-gap-analysis-diagram.md` 完成度 ~50%；`02-phase-d-99-future-evolution.md` 头部写「尚未实现」——与 Phase D～K 交付矛盾。

**范围**：

| 文件 | 动作 |
|------|------|
| `docs/00-roadmap.md` | 重写 §已知限制；新增 Phase L 章节；Gap 表更新完成度 |
| `docs/90-gap-analysis-diagram.md` | Mermaid 节点改为 ✅；完成度表 ~90% |
| `docs/02-phase-d-99-future-evolution.md` | 头部标注「D～K 已交付，下文为历史规划」 |
| `docs/00-PROJECT_STATUS.md` | 增加 Phase L 链接与 next steps |
...
**目标**：一条可 curl / SDK 复现的 **端到端场景**，写入 `docs/90-demo-agent-vertical.md`：
...
- `docs/02-phase-l-30-feedback-loop-e2e.md` 含 live 命令（需 Key）
...
| 交付物 | 说明 |
|--------|------|
| `docs/90-demo-walkthrough.md` | 15 分钟：租户 → RAG v1/v2 金丝雀 → eval compare → Agent vertical → Console（可选） |
| `docs/90-interview-narrative.md` | 10 分钟口述模板 + 诚实边界 + 常见追问 Q&A |
| `eval/platform_demo.sh` | 一键 smoke（分 `--no-llm` / `--with-llm`） |
| Console | `console-v2` build 产出挂 static（若 #62 含 UI，或单列 #63） |

**面试验收**：按 `90-interview-narrative.md` 能讲完 6 层架构 + 2 条深度故事（RAG 发版、Agent 治理）。
...
- [x] RAG：`eval/run.py compare` stub vs 真 rerank（见 `docs/02-phase-l-10-rerank.md`）
...
与 [roadmap.md](./00-roadmap.md) 非目标一致，并额外明确：
...
**诚实边界（1 分钟）**：引用更新后的 `00-roadmap.md` §已知限制。
...
| 文档 | 说明 |
|------|------|
| [issues-backlog-phase-l.md](./99-issues-backlog-phase-l.md) | #53～#63 Issue 正文（GitHub 待创建） |
| [phase-l-priority-roi.md](./02-phase-l-01-priority-roi.md) | ROI 优先级与状态矩阵 |
| [phase-l-console-integration.md](./02-phase-l-31-console-integration.md) | Console 集成跑真 ✅ |
| [demo-walkthrough.md](./90-demo-walkthrough.md) | 15 分钟 Demo 脚本 |
| [enterprise-ai-platform-sop.md](./02-phase-e-sop.md) | 大厂 SOP 对照（RAG 发版、Agent 三率） |
| [phase-d-future-evolution.md](./02-phase-d-99-future-evolution.md) | D4 效果工作流历史规划（#54～#57 继承） |
| [PROJECT_STATUS.md](./00-PROJECT_STATUS.md) | 当前完成度与诚实边界 |
