...
> 规划总览见 [phase-l-engineering-depth.md](./02-phase-l-00-engineering-depth.md)。  
> **ROI 优先级对照**见 [phase-l-priority-roi.md](./02-phase-l-01-priority-roi.md)。
...
**状态**：✅ 已完成（见 [phase-l-console-integration.md](./02-phase-l-31-console-integration.md)）
...
- [ ] 文档 [demo-walkthrough.md](./90-demo-walkthrough.md) 含 SDK 段落
...
Phase A～K 已交付，但 `00-roadmap.md` §已知限制、`90-gap-analysis-diagram.md`、`02-phase-d-99-future-evolution.md` 与代码现状严重矛盾。本 Issue 仅做 **文档同步**，为 Phase L 工程深度 Issue 提供准确基线。

### 验收标准

- [ ] `docs/00-roadmap.md` — 重写 §已知限制（反映 MCP/HITL/PII/语义缓存/Console 等现状）；新增 Phase L 章节链接
- [ ] `docs/90-gap-analysis-diagram.md` — Mermaid 节点与完成度表更新至 ~90%
- [ ] `docs/02-phase-d-99-future-evolution.md` — 头部标注 D～K 已交付，区分历史规划
- [ ] `docs/00-PROJECT_STATUS.md` — next steps 指向 Phase L
...
**修改文件**（无新 packages）：
- `docs/00-roadmap.md`
- `docs/90-gap-analysis-diagram.md`
- `docs/02-phase-d-99-future-evolution.md`
- `docs/00-PROJECT_STATUS.md`
- `README.md`
...
将 `packages/rag/rerank.py` 从词面 stub 升级为可插拔 **Rerank Provider**（HTTP API 如 Cohere/Jina，或 local cross-encoder 占位），支撑 `eval/run.py compare` 量化 rerank 收益。对标 [enterprise-ai-platform-sop.md](./02-phase-e-sop.md) RAG 效果深化。
...
- [ ] `docs/02-phase-l-10-rerank.md` — stub vs api 对比步骤
- [ ] `eval/acceptance_smoke.py` — rerank provider 切换 smoke
- [ ] `docs/00-roadmap.md` — #54 ✅
...
索引任务对未变更 chunk 跳过 embed/upsert，降低 re-index 成本；为 kb 版本 bump 后的快速迭代提供工程基础。继承 [phase-d-future-evolution.md](./02-phase-d-99-future-evolution.md) §6.1。
...
- [ ] 单测 ≥ 10
- [ ] `docs/02-phase-l-11-incremental-index.md`
...
- [ ] 单测 ≥ 12（mock LLM）
- [ ] `docs/02-phase-l-12-llm-judge-eval.md`
- [ ] `docs/00-roadmap.md` — #56 ✅
...
- [ ] `docs/02-phase-l-13-canary-guard.md` — 与 #56 eval 报告联动说明
- [ ] `docs/00-roadmap.md` — #57 ✅
...
- `docs/02-phase-l-13-canary-guard.md`
...
扩展 `eval/agent_run.py`，输出大厂 Agent 效果指标，对标 [enterprise-ai-platform-sop.md](./02-phase-e-sop.md) §Agent L2。
...
- [ ] 单测 ≥ 10
- [ ] `docs/02-phase-l-20-agent-metrics.md`
- [ ] `docs/00-roadmap.md` — #58 ✅
...
- [ ] 场景文档 `docs/90-demo-agent-vertical.md` — curl / SDK 逐步命令
...
- [ ] `docs/00-roadmap.md` — #59 ✅

### 实施计划

**新增/修改**：
- `docs/90-demo-agent-vertical.md`
...
- [x] `docs/02-phase-l-21-agent-eval-gate.md`
- [x] `docs/00-roadmap.md` — #60 ✅
...
- `docs/02-phase-l-21-agent-eval-gate.md`
...
- [x] `docs/02-phase-l-30-feedback-loop-e2e.md` — live 命令清单
...
- [x] `docs/00-roadmap.md` — #61 ✅

### 实施计划

**新增/修改**：
- `eval/feedback_loop_demo.py`
- `docs/02-phase-l-30-feedback-loop-e2e.md`
...
- [x] `docs/90-demo-walkthrough.md` — 全链路步骤与话术（live 勾选 2026-06-23）
- [x] `docs/90-interview-narrative.md` — 10 分钟口述 + Q&A + 诚实边界
- [x] `eval/platform_demo.sh` — `--no-llm` / `--with-llm` 全绿
- [x] `acceptance_smoke.py` 可选 `--platform-demo` 段
- [x] `docs/00-roadmap.md` — #62 ✅

### 依赖

- #62-console ✅
- 软依赖 #57（RAG 回滚故事）、#59（Agent vertical）— 无则 Demo 用 curl 占位
- [ ] `docs/90-interview-narrative.md` — 6 层架构 + 2 深度故事 + Q&A + 诚实边界
...
- [ ] `docs/00-roadmap.md` — Phase L 全部 ✅，打 tag 说明 `phase-l-engineering-depth`
- [x] `docs/02-phase-l-00-engineering-depth.md` — 头部状态改为已完成

### 实施计划

**新增文件**：
- `docs/90-demo-walkthrough.md`
- `docs/90-interview-narrative.md`
...
2. 按 `90-interview-narrative.md` 自检清单勾选
...
5. **更新** [phase-l-engineering-depth.md](./02-phase-l-00-engineering-depth.md) 头部状态
