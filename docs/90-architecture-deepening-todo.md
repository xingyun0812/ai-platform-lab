...
| PR-2 | orchestrator `execution_id` resume 文档化 + 统一只读状态 API | ✅ PR #171 |
...
- [x] orchestrator `execution_id` resume 流程文档化（ADR-0002 §Orchestrator）
- [x] `GET /v1/agent/execution-status` 统一只读 ExecutionHandle（`test_execution_handle.py`）
...
**状态**：✅ · **优先级**：P1 · **关联**：[issues-backlog-phase-r.md](./99-issues-backlog-phase-r.md) · [#146](https://github.com/xingyun0812/ai-platform-lab/issues/146) · tag `arch-self-evolve-146-complete`

### 问题（总述）

- `experience_store` / `long_horizon` store 抽象较深，**生产写路径未闭合**
- R1 模块（reflect / store / retrieve / patch）单测与 smoke 绿，**主路径未接线**
- `99-issues-backlog-phase-r.md` 将 R1 标 ✅，但下列四条实现缺口仍 open
...
| 7b | **Strategy patch 无 REST** | 仅有 Python 级 `approve_strategy_patch()` / `reject_strategy_patch()`；`02-phase-r-01-self-evolving-agent.md` §4 路由为「建议后续」 | `GET/POST /internal/agent/strategy-patches/*`（list / approve / reject），与 HITL 控制台或 ops 脚本对接 |
...
- [ ] 更新 `99-issues-backlog-phase-r.md` 遗留项
...
| 文档 | 用途 |
|------|------|
| [architecture.md](./00-architecture.md) | 现状分层与数据流 |
| [roadmap.md](./00-roadmap.md) | 已知限制 |
| [issues-backlog-phase-r.md](./99-issues-backlog-phase-r.md) | Phase R 与 #7 重叠 |
| [phase-*-build-and-code-guide.md](./02-phase-a-guide.md) | 各 Phase 代码导读 |
| README「文档与代码导读」 | 总索引 |
