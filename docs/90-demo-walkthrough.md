...
> **前置**：[phase-l-console-integration.md](./02-phase-l-31-console-integration.md)（Console 已挂载）  
> **自动化**：`./eval/platform_demo.sh --no-llm`（无 Key）/ `--with-llm`（需 Key + Qdrant）  
> **Live 勾选**（2026-05-19）：见下表 · 闭环 SOP：[closure-sop.md](./00-closure-sop.md)
...
> **内网 LLM 网关**：见 [local-llm-setup.md](./01-local-llm-setup.md)（Base URL、三模型别名、`.env` 模板）。
...
| `DATABASE_URL` | 租户页 **本月使用**、Dashboard token 统计（见 [console-tenant-billing.md](./90-console-tenant-billing.md)） |
...
- 联网搜索：`.env` 设 `WEB_SEARCH_MODE=ddg`，用 `admin` 租户，goal 示例见 [console-agent-planner.md](./90-console-agent-planner.md)

> Phase L #59：见 [demo-agent-vertical.md](./90-demo-agent-vertical.md)（Orchestrator + HITL vertical curl 链，live 6/6 ✅）。
...
详见 [phase-r-agent-harness.md](./02-phase-r-00-agent-harness.md) · [console-tenant-billing.md](./90-console-tenant-billing.md)（租户本月用量）
...
Console 删除文档会调用 purge-source，同步清理 Qdrant + BM25（见 [phase-m-incremental-index.md](./02-phase-m-incremental-index.md)）。
...
点踩 2 条 → `cycle` → 得到 `suggestion_id`（详见 [phase-l-feedback-loop-e2e.md](./02-phase-l-30-feedback-loop-e2e.md)）。

---

## 第 15 分钟：诚实边界 + Q&A

主动说（见 [roadmap.md](./00-roadmap.md) §已知限制、[interview-narrative.md](./90-interview-narrative.md)）：
