...
> 本文档把「面试 + 工程深度」路线整理为 **可执行的优先级**，并与 [issues-backlog-phase-l.md](./99-issues-backlog-phase-l.md) 中的 Issue 编号一一对应。  
> 总规划见 [phase-l-engineering-depth.md](./02-phase-l-00-engineering-depth.md)。
...
|-----------|---------|-------------------|--------------|------------|----------|
| **Console 跑真** | [phase-l-console-integration.md](./02-phase-l-31-console-integration.md) | #62-console（见 backlog） | ❌ 未建 | `console_routes.py`、build、挂载 | ✅ **已完成** |
| **15 分钟 Demo 脚本** | [demo-walkthrough.md](./90-demo-walkthrough.md) | #62 | ❌ 未建 | `eval/platform_demo.sh` | ✅ |
| **SDK 端到端 smoke** | 本文 §第一优先 | #63 | ❌ 未建 | `eval/sdk_smoke.py` | ✅ |
| **面试叙事手册** | [interview-narrative.md](./90-interview-narrative.md) | #62 | ❌ 未建 | — | ✅ |
...
| **Agent 三率** | [phase-l-agent-metrics.md](./02-phase-l-20-agent-metrics.md) | #58 | [#42](https://github.com/xingyun0812/ai-platform-lab/issues/42) | `eval/agent_run.py` | ✅ |
...
| 子任务 | Issue | 工期 | 状态 |
|--------|-------|------|------|
| Console V2 build + 挂载 + 适配 API | **#62-console** | 3～5d | ✅ 完成 → [phase-l-console-integration.md](./02-phase-l-31-console-integration.md) |
| 15 分钟平台 Demo 脚本 | **#62** | 2～3d | ✅ [demo-walkthrough.md](./90-demo-walkthrough.md) + `eval/platform_demo.sh` |
| SDK 端到端 smoke | **#63** | 1～2d | ✅ `eval/sdk_smoke.py` |
| 面试叙事手册 | **#62** | 0.5d | ✅ [interview-narrative.md](./90-interview-narrative.md) |
...
需 LLM Key + live walkthrough 写入 `docs/02-phase-l-30-feedback-loop-e2e.md`。
...
| 文件 | 问题 |
|------|------|
| `00-roadmap.md` §已知限制 | 仍写无 MCP/HITL/PII |
| `90-gap-analysis-diagram.md` | 完成度 ~50% |
| `02-phase-d-99-future-evolution.md` | 头部「尚未实现」 |
| `00-PROJECT_STATUS.md` | Console 状态需更新 |
