...
17. [演进脉络总结](#16-演进脉络总结)
18. [Phase L：工程深度与面试叙事（#53～#63）](#17-phase-l工程深度与面试叙事5363)
19. [Phase M：RAG 增量索引做满（#63～#66）](#18-phase-mrag-增量索引做满6366)
20. [Phase N：Python SDK 发布 PyPI](#19-phase-npython-sdk-发布-pypi)
21. [Phase O：Agent 能力对齐 JD2](#20-phase-oagent-能力对齐-jd2)
22. [Phase P：多模态 Embedding](#21-phase-p多模态-embedding)
23. [Phase Q：任务规划前沿](#22-phase-q任务规划前沿)
24. [Phase R：Agent Harness 前沿](#23-phase-r-agent-harness-前沿)
25. [演进脉络总结（续）](#24-演进脉络总结续)
...
## 17. Phase L：工程深度与面试叙事（#53～#63）

Phase A～K 完成了「平台能力全景」，但存在三类 gap：Stub 未替换（rerank 词面匹配、eval 关键词）、链路未串通（缺端到端演示）、文档滞后。Phase L **不扩新模块**，把 stub / 未验证能力做深、做真、串成故事。

> 总规划：[engineering-depth](./02-phase-l-00-engineering-depth.md) · ROI 优先级：[priority-roi](./02-phase-l-01-priority-roi.md) · Backlog：[issues-backlog-phase-l](./99-issues-backlog-phase-l.md)

### 17.1 L1 — RAG 深化（#54～#57）

#### #54 真 Rerank

Rerank 从词面 stub 升级为三模式可切换：

| 模式 | 环境变量 | 说明 |
|------|----------|------|
| `stub` | `RAG_RERANK_MODE=stub` | 词面重合（默认，无外部依赖） |
| `api` | `RAG_RERANK_API_URL` + Key | HTTP POST `{query, documents}` → `{results:[{index, score}]}` |
| `local` | `RAG_RERANK_MODEL_PATH` | 本地 cross-encoder 模型 |

retrieve 阶段候选先过 rerank → 重排后取 top_n → 继续 min_score 过滤。timings 增加 `rerank_ms`。

#### #55 RAG 增量索引

之前每次索引同一 `source_uri` 全量重建（删旧 + 全部 re-embed）。增量索引用 chunk 指纹跳过未变内容：

```
每个 chunk → sha256(text)[:16] → 写入 Qdrant payload
二次索引 → 对比 offset + content_hash
  → 相同 → 跳过 embed/upsert
  → 新增 → 只 embed 新增
  → 缺失 → 删除对应向量
```

数据量大时节省 60-80% embed 成本。

#### #56 LLM-as-Judge Eval

扩展 eval 判据：
- **keyword**（默认）：`expect` 字段是关键词列表 → answer 包含任一关键词则 pass
- **llm_judge**（可选）：`expect` 字段是 LLM Judge 评分 prompt → 调 LLM 打分 ≥ threshold

```json
{"id": "rag-001", "query": "...", "expect": "答案应包含xxx", "judge": "llm_judge", "threshold": 7}
```

#### #57 KB 金丝雀自动回滚

Phase D4 的金丝雀守卫需要管理员手动检查 eval 报告。L1 让它自动回滚：

```
eval/run.py 跑完 → pass_rate < 0.85 → 写 canary_guard.json
  → routing 层读 canary_guard.json → canary_percent = 0
  → 发送 webhook 通知
  → 记录 canary_auto_rollback metric
```
...
**SOP 闭环**：
1. `python eval/run.py run` 评估新版效果
2. pass_rate < 阈值 → guard 文件自动写入 → 路由层读取 → 流量回切
3. `docker compose logs canary-guard` 查看回滚记录

---

### 17.2 L2 — Agent 深化（#58～#60）

#### #58 Agent 三率指标

之前 Agent eval 只有 pass_rate，缺细粒度指标。Phase L 新增四率：

| 指标 | 含义 |
|------|------|
| **Tool Precision@1** | 第一步是否选对工具 |
| **Needless Tool Rate** | 不该调工具时调了 |
| **Missing Tool Rate** | 该调工具时没调 |
| **Arg Valid Rate** | 工具参数校验成功率 |

四率输出为 `agent_metrics` 报告块，与 pass_rate 并列展示。

#### #60 Agent Baseline + CI 门禁

- `agent_scenarios.jsonl` ≥ 30 条，覆盖 require_tools / direct_answer
- 离线 gate：`eval/agent_gate.py run` 对比当前 vs baseline 四率
- CI 集成：PR 触发时跑 agent_gate.py，回退 >5% → block merge

#### #59 Vertical 用例

Phase H 的 orchestrator + multi-agent + HITL 各自有代码，L2 串联成一条端到端 Agent vertical 用例。

---

### 17.3 L3 — 反馈飞轮闭环 + Console 集成（#61～#63）

#### #61 反馈飞轮 E2E

Phase J #48 的反馈飞轮有模块但缺实测。L3 跑通真实闭环：

```mermaid
flowchart LR
    A["POST /internal/feedback/ 点踩"] --> B["FeedbackStore"]
    B --> C["collect_bad_cases"]
    C --> D["eval 失败"]
    D --> E["LLM 生成 Prompt 建议"]
    E --> F["A/B 实验 50/50 分流"]
    F --> G["Promote 赢家"]
```

#### #62-console Console V2 集成跑真

Console V2 从可访问升级为可演示的 9 页面管理台：Login → Dashboard（统计卡 + 图表）→ Tenants → Agents（委托 + 版本历史）→ RAG（知识库管理 + 查询测试）→ Memory → Orchestrator（工作流 JSON 编辑器 + 执行）→ Audit → Settings。

#### #63 SDK E2E Smoke

SDK 端到端冒烟覆盖 chat / rag / agent / embedding 四条主链路，输出 `eval/sdk_smoke.py` 作为 CI 冒烟。
...
| 决策 | 选型 | 理由 |
|------|------|------|
| Rerank 三模式 | stub/api/local | 开发到生产渐进切换 |
| 增量索引指纹 | sha256[:16] | 轻量、冲突概率极低 |
| Eval 双判据 | keyword + llm_judge | 无 Key CI 能跑，有 Key 更精确 |
| 金丝雀自动回滚 | guard 文件 + routing 读取 | 不改代码、秒级生效 |
| Agent 四率 | 补充 pass_rate | 精准定位 Agent 哪个环节弱 |
| 反馈飞轮人工触发 | AUTO_EXPERIMENT=false | 防止无监督自动上线 |

---

## 18. Phase M：RAG 增量索引做满（#63～#66）

Phase L #55 已在向量侧实现了 chunk 指纹增量，但 BM25 仍是全量重建，删除文档也不会同步清理。Phase M 把增量索引从「向量层可用」升级为 **端到端可演示、可观测、可清理**。
...
之前 BM25 索引每次全量重建（全库 scroll + re-tokenize）。Phase M 改为按 `source_uri` 差量 merge：

- 索引新 `source_uri` → 直接追加
- 再次索引已有 `source_uri` → 删除该 source 的旧 BM25 条目，写入新的 chunk
- 未涉及的 source 保持不变

### M2 — purge-source 清理

`DELETE /internal/index/source/{kb_id}` 删除指定 source 的：

- Qdrant 向量（`filter must source_uri`）
- BM25 条目（按 source_uri 删除）
- 索引任务记录标记为 `purged`

之前 Qdrant 只有 upsert 没有 delete，文档删除后向量变成孤儿。M2 补上删除链路。

### M3 — 增量统计暴露

索引任务响应和 Console 界面现在显示：

| 字段 | 含义 |
|------|------|
| `new_chunks` | 新增 chunk 数 |
| `updated_chunks` | 内容变更的 chunk 数 |
| `skipped_chunks` | 指纹未变跳过的 chunk 数 |
| `deleted_chunks` | 被删除的 chunk 数 |

Prometheus 指标：`index_chunks_new_total` / `skipped_total` / `deleted_total`。

### M4 — Demo 断言

`./eval/platform_demo.sh --with-llm` 二次索引同一 source 后断言 `skipped_chunks >= 1`，确保增量逻辑在 CI 中可验证。

---

## 19. Phase N：Python SDK 发布 PyPI（N1～N4）

把开发者体验从 `pip install -e sdk/python` 升级为 **`pip install ai-platform-lab`**，并形成可重复的发布与 smoke 流程。
...
- `sdk/python/pyproject.toml`：项目元数据、包名 `ai-platform-lab`、Python 3.9+
- `sdk/python/README.md`：安装说明、Client 示例、环境变量
- `__version__` 与 `pyproject.toml` 单源同步

### N2 — CI 发布

`.github/workflows/publish-sdk.yml`，两种触发方式：

- `workflow_dispatch`（人工触发）
- tag `sdk-v*` 自动触发

使用 PyPI **Trusted Publishing**（OIDC）或仓库 Secret `PYPI_API_TOKEN`。

**维护者发版步骤**：

```bash
# 1. bump sdk/python/ai_platform_lab/__init__.py __version__
git tag sdk-v0.1.0 && git push origin sdk-v0.1.0
# 2. CI 自动 build → publish → smoke
```

### N3 — Smoke

`eval/sdk_pypi_smoke.sh`：venv 内 `pip install ai-platform-lab==<ver>` → 跑 `sdk_smoke.py` 验 chat/rag/agent/embedding 四条主链路。CI 可选 job 先用 TestPyPI 验证。

### N4 — 文档同步

- `README.md` 安装段增加 `pip install ai-platform-lab`
- `90-interview-narrative.md` SDK 层更新
- `00-PROJECT_STATUS.md` / `00-roadmap.md` 开发者体验标记 ✅
...
## 20. Phase O：Agent 能力对齐 JD2（O1～O11）

Phase O 对照 JD2 岗位职责，在现有 Agent Runtime 上补齐「规划 / 推理 / 协作 / 外部工具 / 业务场景 / 性能」六块。**面试一句话**：把 Agent 从「ReAct 调工具」升级为可规划、可显式推理、可协作、可接搜索/SQL、可跑数据分析 vertical。

| Issue | 标题 | 状态 |
|-------|------|------|
| O1 | Task Planner + 任务分解 | ✅ #99 |
| O2 | CoT 推理模式 + trace | ✅ #100 |
| O4 | Multi-Agent v2（黑板 + Runner 委托） | ✅ #101 |
| O5 | Plugin Manifest 动态工具 | ✅ #102 |
| O6 | web_search 工具 | ✅ #103 |
| O7 | sql_query 只读工具 | ✅ #104 |
| O9 | 数据分析 Vertical + Orchestrator | ✅ #105 |
| O10 | Agent 性能：并行工具 + 长上下文 | ✅ #106 |
| O11 | 文档 / Demo / eval 门禁 | ✅ #107 |

**执行顺序**：Wave1 O1→O2（Agent 大脑）→ Wave2 O4（协作）→ Wave3 O5→O6→O7（外部能力）→ Wave4 O9（vertical）→ Wave5 O10（性能）→ Wave6 O11（文档+tag）

### O1 — Task Planner

新模块 `packages/agent/planner.py`：输入 goal + context → LLM 生成结构化 Plan JSON（`steps[]: {id, description, tool_hint, depends_on}`）。API `POST /v1/agent/plan` 或 `auto_plan: true` 内嵌。

```python
class Plan:
    steps: list[PlanStep]  # id, description, tool_hint, agent_hint, depends_on
```

Plan 逐步喂给 runner 执行，支持拓扑校验（拒绝循环依赖）。Console 显示 Plan 轨迹 JSON。

### O2 — CoT 推理模式

在默认 `react` 基础上新增 `cot` 模式：Agent 在 `<thinking>...</thinking>` 中写出可见的推理 trace，再返回正文或工具调用。配置三层覆盖：`config/agent.yaml` → `AGENT_REASONING_MODE` 环境变量 → 请求体 `reasoning_mode` 字段。
...
2. **共享黑板** — 委托输出写入 `blackboard:{tenant}:{session}`，子 Agent 可读前序结果
3. **Reviewer 模式** — `role=reviewer` 自动读取黑板再裁决

### O5 — Plugin Manifest

轻量 YAML 插件体系：把 Agent 工具声明放在 `config/plugins/*.yaml`，Gateway 启动时自动注册到 `ToolRegistry`。与 MCP（远程 Server）互补：MCP = 远程进程，Plugin = 本地 manifest 文件。

### O6 — web_search 工具

Agent 内置工具 `web_search`：搜索公开互联网信息，返回结构化 `{title, snippet, url}` 列表。支持 mock / ddg（DuckDuckGo）/ http 三模式。天气类 query 自动调用 Open-Meteo 补充实时数值。

### O7 — sql_query 只读工具

Agent 内置工具 `sql_query`：受控只读 SQL，仅允许 `SELECT`，自动强制 `LIMIT`，拒绝 DML/DDL。mock 模式返回示例数据，postgres 模式直连只读连接。

### O9 — 数据分析 Vertical

端到端 workflow：**web_search（背景）→ sql_query（聚合）→ calc（同比）→ 报告摘要**。Orchestrator 工作流定义在 `config/workflows/data_analysis.yaml`，Agent spec `data_analyst` 在 `config/agents.yaml`。
...
| 优化 | 效果 |
|------|------|
| 并行工具执行 | `tool_call_strategy=parallel` 同一轮多个 tool_calls `asyncio.gather` 并发 |
| 长上下文策略 | `context_token_budget` 上限 + `tool_result_max_chars` 截断 |
| Prometheus 指标 | `agent_tool_parallel_duration_ms` / `agent_context_tokens_total` |

### O11 — 文档 / Demo / eval 门禁

- `eval/agent_jd2_gate.py` 作为 Phase O 门禁
- Console Agents 页展示 Planner 轨迹、Multi-Agent 委托记录
- 面试叙事稿同步更新

---

## 21. Phase P：多模态 Embedding（P1～P4）

在 Phase G Embedding 独立服务基础上扩展 **text + image** 统一输入，支持 OpenAI 风格 `inputs`、stub 确定性向量、RAG 图文索引。
...
`POST /internal/embeddings/embed` 新增 `inputs` 参数，兼容已有 `texts`：
...
- `packages/embedding/multimodal.py`：归一化 + 指纹缓存
- `EmbeddingModel.modalities` 字段声明模型支持的能力
- 新预置模型 `stub-multimodal`：确定性哈希向量，CI 零依赖

### P2 — RAG 图文索引

- `packages/rag/multimodal_index.py`：图片检测 + caption sidecar
- `TextChunk` 新增 `modality` 字段，`content_fingerprint` 增量跳过
- 图片走 `chunk_image_file`，存储 image_base64 到向量库
- 配置项 `RAG_MULTIMODAL_EMBEDDING_MODEL`

### P3 — Console / SDK

Console Embedding 页面和 SDK `EmbeddingResource.create()` 支持 `inputs` 格式。

### P4 — 门禁

`eval/multimodal_embedding_gate.py run` 验证图文 embed → RAG 索引 → 相似度检索全链路。

---

## 22. Phase Q：任务规划前沿（Q0～Q7）

Phase O O1 已经能做到：给一个 goal → LLM 生成 Plan JSON（步骤列表 + 依赖关系）→ 按拓扑序**逐个 step 串行**执行。但串行意味着互不依赖的步骤也要排队等，失败直接中断，没有质量保障。Phase Q 的目标是：在不大改 ReAct Runtime 的前提下，把 Planner 从**「能规划」**升级到 **「可重规划、可并行、可门禁」**。

| Wave | Issue | 标题 | 解决什么问题 |
|------|-------|------|-------------|
| 1 | Q1 | Structured Plan 输出 | O1 用 Prompt 解析 JSON，不稳定；改模型原生结构化输出 |
| 1 | Q2 | DAG 并行 step 执行 | 串行步骤互相阻塞，fan-out 场景慢 |
| 2 | Q3 | 失败重规划 Critic | 一步失败整个 Plan 就废了，缺局部修复 |
| 2 | Q4 | Plan 级 HITL | 执行前没人审 plan，高风险任务不放心 |
| 3 | Q5 | Planner ↔ Orchestrator 桥接 | Planner 产出的 Plan 和 Orchestrator 工作流两套东西，不能互通 |
| 4 | Q6 | 规划质量门禁 | 没有 baseline，改 planner 逻辑后不知道质量是否退化 |
| — | Q7 | Graph Runtime | 多个 checkpoint/resume 机制分散在各处，缺统一入口 |

### Q1 — Structured Plan 输出

O1 的 Planner 靠 Prompt 引导 LLM 输出 JSON，再用 `extract_json_object()` 正则解析。问题是：模型输出不稳定，偶尔 JSON 格式不对就解析失败。

Q1 改为优先走 upstream 的 `response_format: json_schema`——这是 OpenAI 兼容 API 的原生能力，让模型直接输出符合 schema 的结构化 JSON，无需正则解析。降级保留 `extract_json_object` 兜底。Plan 解析成功率从"看模型心情"变成接近 100%。

### Q2 — DAG 并行执行

O1 的 `execute_plan_with_agent()` 按拓扑序逐个 step 串行执行。假设 Plan 有 4 个 step：A→B、A→C、B→D、C→D。A 执行完后 B 和 C 互不依赖，但 O1 会等 B 跑完再跑 C——白白等待。

Q2 的做法：

```
topological_sort → 按层分组
  第 1 层: [A]
  第 2 层: [B, C]         ← B 和 C 同时跑
  第 3 层: [D]
```

层内 `asyncio.gather` 并发，每步用独立 sub-session ID 避免黑板写冲突。关键策略 **fail-open**：某步失败不阻塞同层其他步骤（但 `pending_approval` 停掉后续层，HITL 优先）。

### Q3 — Replan Critic

O1 中某 step 失败（比如工具抛异常），整个 Plan 就断了。Q3 引入 Critic LLM 做局部修复：

```
step 失败 → attempt < max_replan_attempts(默认2)?
  ├─ Yes: 调 Critic LLM
  │  输入: 原 plan + 失败 step + 失败原因
  │  输出: 修订后的完整 plan（只改失败附近）
  │  记录 plan_revisions[] → 递归重执行
  └─ No: 终止，status=failed
```

每次修订的 trigger、failed_step、critic_summary 都记录在案，执行轨迹可追溯。

### Q4 — Plan 级 HITL

Phase E 有**工具级 HITL**（高风险工具暂停等人批），但 Plan 本身没人审——Agent 决定"做这三步"就直接做了。对于高风险场景（比如"删掉数据库"），应该在执行前让人看一遍 Plan。

```
generate_plan() → require_plan_approval=True
  → 返回 { status: "pending_plan_approval", plan_approval_id, plan }
  → Console 展示步骤树: Step1 → Step2 → Step3
  → 批准 → POST approve → 重触发执行
  → 拒绝 → POST reject → Plan 终止
```

### Q5 — Planner ↔ Orchestrator 桥接

Planner 产出 `AgentPlan`（goal + steps + depends_on），Orchestrator 接受 `config/workflows/*.yaml`（nodes + edges）。两者结构不同，不能互通。

`plan_to_workflow(plan)` 把 AgentPlan 转成 workflow spec，让 Planner 输出可直接被 Orchestrator 执行。`POST /v1/agent/plan/export` 可下载 YAML，人工修改后放回 `config/workflows/`。

### Q6 — 规划质量门禁

改了 Planner 逻辑有没有变差？Q6 建了离线评测基线：

`eval/plan_baseline.jsonl` 每条用例：`goal` + `min_steps` + `max_steps` + `required_tool_hints`。

`check_plan_quality()` 检查三项：步骤数在范围内 → 工具提示满足 → 无循环依赖。Mock 模式离线可跑，CI 零外部依赖。

### Q7 — Graph Runtime

Plan 审批 resume、Orchestrator checkpoint 散在各处，调用方需要知道太多细节。Q7 提供 `execute_agent_graph()` 统一入口，整合这些机制。

**与 Phase R2 边界**：Q7 管"怎么跑起来"（编排层），R2 管"任务跑多久"（业务任务层）。`execution_id` ≠ `task_id`，API 字段不混用。

---

## 23. Phase R：Agent Harness 前沿（R0～R4）

### 为什么 Phase R 存在？

Phase A～Q 交付了完整的 AI 中台骨架，但对照 DeepSeek Harness 团队的 JD 与业界前沿，还有三个核心缺口：

**缺口 1：Agent 不会自我进步。** Phase J 的反馈飞轮只做了一件事——用户点踩 -> 收集 bad case -> 生成 Prompt 修改建议。但改完 Prompt 后，Agent 下次遇到类似任务还是从头开始，不会说"上次这个任务我成功了，这次可以复用那个经验"。就像一个实习生每次做同样的工作都重新学一遍。

**缺口 2：任务不能跨 session。** Phase Q 的 Plan + Replan 有 `max_replan_attempts=2` 的限制，而且所有状态在 session 结束后就丢了。如果 Agent 正在执行一个需要 10 分钟的多步 Plan，用户中途关掉了浏览器，回来时一切归零。更别说跨天执行的长任务了。

**缺口 3：不知道哪个模型适合什么。** Model Router 只做简单的路由和降级——请求来了选一个 provider，失败了换另一个。但它不知道"gpt-4o 长上下文好但规划差"、"claude-3-5-sonnet 规划好但长上下文不如 gpt-4o"。没有数据反哺路由策略。

### 一句话面试讲法

> Phase R 让 Agent 能**沉淀经验、跨天执行、自知模型强弱**——对齐 DeepSeek Harness 团队的三个研究方向：自进化、长程任务、能力探测。

| Wave | Issue | 标题 | 解决什么问题 |
|------|-------|------|-------------|
| 1 | R1 | 自进化 Agent | Agent 每次完成任务后沉淀经验，下次相似任务复用 |
| 1 | R2 | 跨 session 长程任务 | Plan 可以跨天执行，随时挂起随时续跑 |
| 2 | R3 | 模型能力探测 | 自动测出每个模型在 4 个维度的强弱，反哺路由 |
| 2 | R4 | eval 门禁 + tag | 三个维度的联合评测，防止退化 |

执行路径：R1 -> R2（Wave 1 可面试演示）-> R3 -> R4（Wave 2 偏工程深度）。

---

### R1 — 自进化 Agent（#134）

**你遇到过的问题**：反馈飞轮只改了 Prompt，但 Agent 不会积累经验。比如昨天 Agent 成功完成了一个"分析销售数据"的任务——它搜了背景信息、查了数据库、算了同比、写了报告。今天又来一个"分析用户增长"的任务，Agent 又要重新想怎么做，不会说"上次做数据分析的套路可以用"。

**R1 怎么解决**：Agent 每次执行完任务后，把成功路径沉淀为一条"经验"存到经验库。下次遇到相似任务时，先查经验库，有匹配就把历史成功经验注入到 Planner 的 prompt 里。

```
Agent 跑完任务
  -> _reflect_on_run(plan, outcome)    <- LLM 反思：这次哪里做得好？
    -> 生成 ExperienceRecord
      task_signature: SHA1(goal)[:16]  <- 任务"指纹"用于相似匹配
      plan: 成功的 Plan
      tool_calls: 调了哪些工具
      lessons: "数据分析类任务应先 web_search 查背景，再 sql_query 取数"
  -> 存到 experience_store

下次新任务
  -> generate_plan(goal, ...)
    -> 先查 experience_store.retrieve_similar(goal, top_k=3)
    -> 有匹配 -> 把 lessons 注入 plan_prompt
    -> 生成 Plan
```

**策略自改**：如果 Agent 连续多次在同类任务上失败，`_maybe_patch_strategy()` 会生成一个 `StrategyPatch`，修改 plan_prompt 或 tool_selection 策略。这个 patch **必须走 HITL**——不能静默改，需要人工审批后才生效。

**设计原则**：
| 原则 | 说明 |
|------|------|
| 经验优先 | 相同 task_signature 时优先注入历史成功经验 |
| HITL 守门 | 策略 patch 入库 -> 等人批 -> 不直接改代码 |
| 异常隔离 | 所有自进化步骤 try/except，失败只 log warning |
| 无副作用 | 经验注入只影响 prompt context，不改变 plan 逻辑 |

**验收**：`eval/self_evolve_smoke.py` 跑同一类任务 2 次，第 2 次应复用经验。

---

### R2 — 跨 session 长程任务（#135）

**你遇到过的问题**：Phase Q 的 Plan 执行是"一把梭"——Plan 生成了就一口气执行完，中间不能停。假设 Plan 有 8 个 step 需要跑 10 分钟，用户等不了那么久，关掉了页面。回来时全丢了，要从头开始。

更真实场景：一个数据分析任务——搜数据 -> 清洗 -> 建模 -> 出报告。每步可能几分钟。用户希望：早上提交任务 -> 中午回来看看进度 -> 下午出报告。中间可以关掉浏览器不管。

**R2 怎么解决**：把 Plan 执行变成"可暂停、可续跑"的。

```
POST /v1/agent/long-run     <- 创建一个长程任务
  { goal: "分析销售数据" }
  -> 返回 task_id
  -> 后台开始执行
  -> 每完成一层 DAG，自动 checkpoint：
    checkpoint(task_id):
      持久化 step_states（已完成/进行中/失败）
      持久化中间产物（tool_calls、messages）
      更新 status

用户随时：
  GET /v1/agent/long-run/{task_id}    <- 查进度（如 "3/8 steps 完成"）
  POST /v1/agent/long-run/{task_id}/cancel  <- 取消

回来继续：
  POST /v1/agent/long-run/{task_id}/resume  <- 加载最新 checkpoint
    -> 跳过已完成的 step
    -> 从下一个未完成的 step 继续
```
...
| 层 | 实现 | 条件 |
|----|------|------|
| 内存 | `InMemoryLongRunTaskStore` — dict + RLock | 默认/开发/测试 |
| Postgres | `PostgresLongRunTaskStore` — psycopg v3 + JSONB | `DATABASE_URL` 已配置 |
| Redis 缓存 | `RedisLongRunCache(base)` — 装饰器模式加速读 | `REDIS_URL` 已配置 |

自动选择策略：有 DATABASE_URL -> 尝试 Postgres，失败 warn 降级到内存；有 REDIS_URL -> 叠加缓存。

**与 Q7 边界**（面试常问）：
| 概念 | 属于 | 主键 | 管什么 |
|------|------|------|--------|
| Plan 审批挂起 | Q7 Graph Runtime | `plan_approval_id` | 生成 Plan 后等人批 |
| 工作流节点断点 | Q7 Orchestrator | `execution_id` | workflow 节点级 resume |
| 长程 Plan 任务 | **R2** | `task_id` | **跨 session 的 Plan，管理员查进度/取消** |

用户视角一句话：批 Plan 用 `plan_approval_id`；跑 workflow 看 `execution_id`；跨天的长任务用 `task_id`。

---

### R3 — 模型能力探测（#136）

**你遇到过的问题**：Model Router 选 provider 的策略很简单——balanced 按价格和延迟加权打分。但它不知道不同模型在 AI 能力维度上的差异。比如 gpt-4o 长上下文做得好但规划能力一般，claude-3-5-sonnet 规划很强但长上下文不如 gpt-4o。没有数据，路由策略就没办法做"按任务类型选模型"的优化。

**R3 怎么解决**：自动对每个模型跑 4 个维度的 benchmark，输出一份能力探测报告。

| 维度 | 测什么 | 典型用例 |
|------|--------|---------|
| **Context Management** | 长文本 recall、position bias | 10 万 token 的文档中能否准确找到关键信息 |
| **Long Memory** | 跨轮记忆、摘要准确度 | 对话 20 轮后还记得第 3 轮提到的细节吗 |
| **Tool Use** | function calling 准确率、参数合规 | 给一个复杂 schema，能正确填参数吗 |
| **Planning** | Plan 合理性、重规划质量 | 多步任务的步骤分解和失败恢复能力 |

输出报告样例：

```
| 模型 | context | memory | tool | planning | overall | 最强 | 最弱 |
|------|---------|--------|------|----------|---------|------|------|
...
这个报告反哺 Model Router：当任务类型是"长文档分析"时优先路由到 context 强的模型；当任务是"多步规划"时选 planning 强的模型。

**诚实边界**：当前为 mock 模式（预设 scoring 逻辑，非真实 LLM 调用），真实 benchmark 需要实际调模型 API。

---

### R4 — Eval 门禁（#137）

`eval/harness_capability_gate.py run` 把 R1/R2/R3 三个维度的评测联合起来：

| 评测维度 | 对应 |
|---------|------|
| 自进化 | 同一任务跑 2 次，第 2 次是否能复用经验 |
| 长程任务 | checkpoint 后 resume 能否正确跳过已完成 step |
| 能力探测 | 4 维度 benchmark 是否能稳定输出报告 |

相对 baseline 回退 >5% -> gate fail -> exit 1 -> CI block merge。

## 24. 演进脉络总结（续）

| 阶段 | 核心交付 | 工程成熟度 |
|------|---------|-----------|
| Phase L | RAG/Agent 深化 + Console 集成 + 面试叙事 | Stub → 真实现，模块 → 端到端故事 |
| Phase M | BM25 增量 merge + 删除清理 + 统计暴露 | 增量索引端到端可演示/可观测 |
| Phase N | Python SDK PyPI 发布 + CI smoke | pip install 即用 |
| Phase O | Planner / CoT / Multi-Agent / 搜索/SQL / 性能 | Agent 能力对齐 JD2 |
| Phase P | 多模态 Embedding + RAG 图文索引 | Text+Image 统一 Embedding |
| Phase Q | DAG 并行 / Replan / Plan HITL / 质量门禁 | 任务规划前沿对齐 |
| Phase R | 自进化 Agent / 长程任务 / 模型能力探测 | Agent Harness 研究前沿 |

至此，平台从 Week1 的 Gateway 起步，经过 18 个 Phase（A～R）的持续演进，覆盖了模型网关、RAG（含增量与图文）、Agent（Planner/CoT/Multi-Agent/Harness）、观测评测、硬化、Prompt/MCP/Embedding/HITL、安全合规、SDK/PyPI、Console V2、生产 Helm、架构加深等全部核心能力。