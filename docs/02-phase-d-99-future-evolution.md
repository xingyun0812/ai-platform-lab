...
> **状态**：**历史规划文档**。Phase D～K **已按 roadmap 交付**（见 [roadmap.md](./00-roadmap.md)、[PROJECT_STATUS.md](./00-PROJECT_STATUS.md)）。  
> **当前焦点**：Phase L — 工程深度与面试叙事（[phase-l-engineering-depth.md](./02-phase-l-00-engineering-depth.md)），把 stub / 未 live 验证能力做深。  
> **定位**：本仓库仍是 **学习 + 面试可讲的最小中台**；下文 D～K 段落保留为 **当时的产品演进思路**，不等于待办清单。

构建思路、使用链路与逐文件代码说明见 [phase-d-build-and-code-guide.md](./02-phase-d-00-guide.md)。
...
- [roadmap.md](./00-roadmap.md) — 已完成阶段与诚实边界  
- [phase-c-platform.md](./02-phase-c-platform.md) — Phase C 交付说明  
- [architecture.md](./00-architecture.md) — 分层与数据流  
- [AI中台学习执行手册.md](./90-ai-learning-handbook.md) — 8 周学习主线  
