"""LLM 上游调用 — OpenAI 兼容 /chat/completions（Issue #150）。"""
...
logger = logging.getLogger("ai_platform.llm")
...
async def forward_chat_completions(
    ...
    *,
    ...
    key = (api_key or getattr(settings, "llm_api_key", "") or "").strip()
    if not key:
        return 503, None, "LLM_API_KEY 未配置：申请到账号后写入 .env 即可联调"

    base = (base_url or getattr(settings, "llm_base_url", "")).rstrip("/")
    ...
    max_retries = max(0, int(getattr(settings, "upstream_max_retries", 0)))
    timeout = httpx.Timeout(float(getattr(settings, "upstream_timeout_seconds", 60.0)))
